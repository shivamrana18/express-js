...
const schema = require( '../models/sampleSchema' )
...
const { body, validationResult } = new validator.ExpressValidator(
  {
    isPostID: async value => {
      // Verify if the value matches the post ID format
    },
...
router.route( '/getFile' ).get( ( req, res ) => {
  res.status( 200 ).json( {
    messgae: `Get file using GET request`
  } ).sendFile( '/src/Config/dbConnecttion.js', () => {
...
// check validations based on schema
router.route( '/createUser' ).post( validator.checkSchema( schema ), ( req, res ) => {
